...
class DashboardComplaintTrendEndpointTests(APITestCase):
    fixtures = ["lenders.json", "borrowers.json", "loans.json", "complaints.json"]

    def test_returns_cumulative_complaint_counts_grouped_by_day(self):
        response = self.client.get("/api/dashboard/complaint-trend/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIsInstance(response.data, list)
        self.assertGreater(len(response.data), 0)
        row = response.data[0]
    ...
    def test_counts_sum_to_total_complaints(self):
        ...
        response = self.client.get("/api/dashboard/complaint-trend/")
        total_from_endpoint = response.data[-1]["complaint_count"]
        self.assertEqual(total_from_endpoint, Complaint.objects.count())
