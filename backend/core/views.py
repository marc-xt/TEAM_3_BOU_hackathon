...
@api_view(["GET"])
def dashboard_complaint_trend_view(request):
    """GET /api/dashboard/complaint-trend/ -> cumulative complaint counts
    grouped by day. Consumed by Dev 2's Complaint Trend chart."""
    rows = (
        Complaint.objects.annotate(date=TruncDate("created_at"))
        .values("date")
    ...
    for row in rows:
        if row["date"] is None:
            continue
        running_total += row["complaint_count"]
        data.append(
            {
                "date": row["date"].isoformat(),
                "complaint_count": running_total,
            }
        )
